...
MongoClient.connect('mongodb://localhost:27017/mongomart', function(err, db) {
    ...
    router.get("/cart", function(req, res) {
        // redirects to the 'router.get("/user/:userId/cart")' 
    ...
    // ROUTE for the 'Add To Cart' button
    router.post("/user/:userId/cart/items/:itemId", function(req, res) {
        "use strict";
        console.log('Ruta para añadir al carrito desde el boton -AddToCart-');
        ...
        cart.itemInCart(userId, itemId, function(item) {
            if (item == null) {
                // si el item no existe en el carrito se inserta
                items.getItem(itemId, function(item) {
                    item.quantity = 1;
                ...
                // el item existe en el carrito, se actualiza la cantidad
    ...
    // ROUTE for updating the quantity of an item WHEN using "the quantity selector"
    router.post("/user/:userId/cart/items/:itemId/quantity", function(req, res) {
        "use strict";
        console.log('Ruta /user/:userId/cart/items/:itemId/quantity');
